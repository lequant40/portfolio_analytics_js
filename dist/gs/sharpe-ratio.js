 ...
 */
  ...
  * // XXX
  */
  function sharpeRatio(equityCurvePortfolio, equityCurveBenchmark) {
    // No need for input checks, as done in function below
	
	// Compute the arithmetic returns of the portfolio
	var portfolioReturns = arithmeticReturns(equityCurvePortfolio).slice(1); // First value is NaN

	// Compute the arithmetic returns of the benchmark
	var benchmarkReturns = arithmeticReturns(equityCurveBenchmark).slice(1); // First value is NaN
	...
	return sharpeRatio_(differentialReturns);	
  }
  ...
  * // XXX
  */
  function sharpeRatio_(differentialReturns) {
  ...
  * // XXX
  */
  function biasAdjustedSharpeRatio(equityCurvePortfolio, equityCurveBenchmark) {
    // No need for input checks, as done in function below
	...
	var s = sharpeRatio_(differentialReturns);
	
	// Then compute its small sample bias, c.f. formula 11b of the reference
	var bias = 1 + 0.25 * (sampleKurtosis_(differentialReturns) - 1)/portfolioReturns.length;
  ...
  //sharpeRatioConfidenceInterval (alpha)
